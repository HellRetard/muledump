...
var SLOT_ORDER = [1,2,3,17,8,24, 14,6,7, 4,5,11,12,13,15,16,18,19,20,21,22,23,25, 9, 10, 26, 100, 0];
...
function update_totals() {
	...
	function upd(arr) {
		...
		for (var i = 0; i < arr.length; i++) {
			...
			if (items[id][7] > 0)
				totals[id] = stackCount(id);
			// This is bad because if you have 3 stacks of the same token type
			// you will see 3 stacks in totals and they won't sum up together,
			// but honestly do we even need that? You are mostly about to stack
			// them up and then use it to get rewards from the tokens
			else if (id in totals) totals[id]++; else totals[id] = 1;
		}
	}
